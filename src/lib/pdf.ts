...
/** Prontuário completo em PDF, retornado como Blob (para upload/compartilhamento). */
export function pdfMedicalRecordBlob(opts: {
  vet: VetInfo; pet: Pet; client: Client | null; record: MedicalRecordData;
}): Blob {
  const doc = new jsPDF();
  const r = opts.record;
  header(doc, "Prontuário", opts.vet);
  infoBlock(doc, opts.pet, opts.client, 40);

  let y = 58;
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("Data do atendimento:", 14, y);
  doc.setFont("helvetica", "normal");
  doc.text(new Date(r.appointment_date).toLocaleString("pt-BR"), 62, y);
  y += 6;
  doc.setFont("helvetica", "bold");
  doc.text("Peso / Temperatura:", 14, y);
  doc.setFont("helvetica", "normal");
  doc.text(`${r.weight ? r.weight + " kg" : "—"}  /  ${r.temperature ? r.temperature + " °C" : "—"}`, 62, y);
  y += 10;
  ...
  const section = (title: string, body?: string | null) => {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    const lines = doc.splitTextToSize(body, 180) as string[];
    lines.forEach((line) => {
  ...
  section("Anamnese", r.anamnesis);
  section("Sintomas", r.symptoms);
  section("Diagnóstico", r.diagnosis);
  section("Prescrição", r.prescription);
  section("Observações", r.observations);

  footer(doc, r.signature_url ?? undefined);
