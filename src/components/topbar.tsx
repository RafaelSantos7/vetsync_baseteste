...
import { Search, LogOut, RefreshCw, WifiOff, Building2 } from "lucide-react";
...
export function Topbar() {
  ...
  const { data: orgs } = useRxQuery("organizations", { selector: { id: { $eq: organizationId } } });
    ...
    <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-background/70 px-4 backdrop-blur">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <div className="hidden text-right text-xs leading-tight sm:block">
          <div className="font-medium capitalize">{user?.user_metadata?.full_name ?? user?.email}</div>
          <div className="flex items-center justify-end gap-1 text-muted-foreground uppercase text-[10px] font-bold">
            {activeOrg && <Building2 className="h-2.5 w-2.5" />}
            {activeOrg?.name || (user?.user_metadata?.app_role ? user.user_metadata.app_role : "Veterinário")}
          </div>
        </div>
