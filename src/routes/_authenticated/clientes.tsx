...
const empty: Omit<Client, "id"> = {
  name: "", phone: "", whatsapp: "", email: "", document: "", address: "", city: "", notes: "",
};
...
function ClientesPage() {
  ...
  const { data: rawClients = [], isLoading: rxLoading } = useRxQuery<Client>("clients", {
    selector: { 
      is_deleted: { $ne: true },
  ...
  function openEdit(c: Client) {
    ...
    setForm({
      ...
      email: c.email ?? "", document: c.document ?? "",
    ...
    <div className="space-y-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-xl w-[95dvw] h-[90dvh] sm:h-auto overflow-y-auto">
              ...
              <DialogTitle className="flex items-center justify-between">
                ...
                {editing && (
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={(e) => { e.stopPropagation(); remove(editing); setOpen(false); }}
            ...
            <div className="grid gap-4 sm:grid-cols-2 pb-20 sm:pb-0">
              <Field label="Nome *" value={form.name} onChange={(v) => setForm({ ...form, name: v })} className="sm:col-span-2" />
              ...
              <Field label="Endereço" value={form.address ?? ""} onChange={(v) => setForm({ ...form, address: v })} className="sm:col-span-2" />
      ...
      {isLoading ? (
        ...
        <div className="glass flex flex-col items-center justify-center gap-3 rounded-xl p-12 text-center shadow-elegant">
          ...
          <Button onClick={openNew} size="sm" className="gap-1.5"><Plus className="h-4 w-4" /> Cadastrar primeiro</Button>
        </div>
      ) : (
              ...
              <div className="flex flex-col h-full justify-between">
                <div>
                  ...
                  <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                    {c.phone && <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-primary/70" /> {c.phone}</div>}
                    {c.email && <div className="flex items-center gap-2"><Mail className="h-4 w-4 text-primary/70" /> {c.email}</div>}
                    {c.city && <div className="flex items-center gap-2"><MapPin className="h-4 w-4 text-primary/70" /> {c.city}</div>}
                  </div>
                </div>
                <div className="mt-4 flex items-center justify-between pt-3 border-t border-border/50">
                  <span className="text-xs font-medium text-primary/80 group-hover:text-primary">Ver detalhes</span>
                  <div onClick={(e) => { e.stopPropagation(); remove(c); }}>
