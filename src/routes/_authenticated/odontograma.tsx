...
type ToothRow = {
  id: string; odontogram_id: string; tooth_number: number;
  status: ToothStatus; procedure: string | null; notes: string | null;
  images?: string[] | null;
};
...
function OdontogramaPage() {
  ...
  const createOdo = async (petId: string | null) => {
    ...
    try {
      ...
      setOdoId(id);
      if (!petId) toast.message("Ficha criada sem paciente", { description: "Lembre-se de vincular a um animal cadastrado." });
  ...
  const upsertTooth = async (n: number, patch: Partial<ToothRow>) => {
    ...
    if (existing?.id) {
      ...
      await toothCol.insert({
        id: uuid(),
        odontogram_id: odoId,
        tooth_number: n, status: "sadio", procedure: null, notes: null, images: [],
        ...patch,
        is_deleted: false, created_at: now, updated_at: now,
      });
    }
  ...
  const exportPDF = () => {
    ...
    doc.setFillColor(20, 30, 48); doc.rect(0, 0, 297, 24, "F");
    doc.setTextColor(255, 255, 255); doc.setFont("helvetica", "bold");
    doc.setFontSize(15); doc.text("FICHA ODONTOLÓGICA EQUINA", 14, 12);
    doc.setFontSize(10); doc.setFont("helvetica", "normal");
    doc.text(`Paciente: ${pet?.name ?? "— sem vínculo —"}  •  ${pet?.breed ?? ""}  •  ${pet?.sex ?? ""}`, 14, 19);
    doc.text(`Data: ${new Date(odo?.exam_date ?? Date.now()).toLocaleDateString("pt-BR")}`, 240, 19);
    doc.setTextColor(0, 0, 0);

    let y = 34;
    doc.setFont("helvetica", "bold"); doc.setFontSize(11);
    doc.text("Dentes registrados (Triadan):", 14, y); y += 7;
    ...
    if (!rows.length) {
      ...
      doc.setFillColor(240, 240, 245); doc.rect(14, y - 4, 269, 7, "F");
      doc.setFont("helvetica", "bold");
      doc.text("Dente", 18, y); doc.text("Status", 50, y);
      doc.text("Procedimento", 100, y); doc.text("Observações", 180, y);
      y += 6; doc.setFont("helvetica", "normal");
      [...rows].sort((a, b) => a.tooth_number - b.tooth_number).forEach((t) => {
    ...
    if (odo?.notes) {
      y += 6; doc.setFont("helvetica", "bold"); doc.text("Notas gerais:", 14, y); y += 5;
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Triadan — clique no dente para registrar procedimento</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {pets.length > 0 && (
              <CreateWithPet pets={pets} onCreate={(id) => createOdo(id)} />
            )}
          </div>
        </div>
                ...
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {!pet && <Badge variant="outline" className="border-amber-500/40 text-amber-500 text-[10px]"><AlertTriangle className="mr-1 h-3 w-3" />vincular</Badge>}
                      {pet?.breed && <span className="text-xs text-muted-foreground">{pet.breed}</span>}
                    </div>
                    <p className="text-xs text-muted-foreground">{new Date(o.exam_date).toLocaleString("pt-BR")}</p>
                    {o.notes && <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{o.notes}</p>}
                  </div>
      ...
      {odoId && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card/60 p-3">
            <Button variant="ghost" size="sm" onClick={() => setOdoId(null)}>← Voltar</Button>
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <span className="font-medium">{currentPet?.name ?? "— Sem paciente —"}</span>
                {currentPet ? (
                  <Badge variant="outline" className="text-[10px]">{currentPet.breed ?? ""}</Badge>
                ) : (
                  <Badge className="bg-amber-500/15 text-amber-500 text-[10px]"><AlertTriangle className="mr-1 h-3 w-3" />sem paciente vinculado</Badge>
                )}
              </div>
              <p className="text-xs text-muted-foreground">{currentOdo && new Date(currentOdo.exam_date).toLocaleString("pt-BR")}</p>
            </div>
            ...
            <Button onClick={exportPDF} size="sm" variant="outline"><FileDown className="mr-1 h-4 w-4" />PDF</Button>
            <Button onClick={removeOdo} size="sm" variant="outline"><Trash2 className="mr-1 h-4 w-4 text-destructive" /></Button>
          </div>

          {!currentPet && pets.length === 0 && (
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-amber-300">
              Você ainda não tem animais cadastrados. Você pode trabalhar nesta ficha normalmente, mas
              ao final lembre-se de cadastrar o equino em <strong>Animais</strong> e vincular esta ficha.
            </div>
          )}
      ...
      <Dialog open={linkOpen} onOpenChange={setLinkOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Vincular ficha a um paciente</DialogTitle></DialogHeader>
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => linkPet(v)}>
              ...
              <SelectContent>
                {pets.map((p) => <SelectItem key={p.id} value={p.id}>{p.name} {p.breed ? `· ${p.breed}` : ""}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        </DialogContent>
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Nova ficha odontológica</DialogTitle></DialogHeader>
          <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button disabled={!val} onClick={() => { onCreate(val); setOpen(false); setVal(""); }}>Criar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
    ...
    <div className="rounded-xl border border-border bg-card/60 p-4">
      ...
      <div className="mt-2 flex justify-end">
        <Button size="sm" variant="outline" onClick={async () => {
          ...
          if (doc) await doc.incrementalPatch({ notes: val, updated_at: new Date().toISOString() });
          toast.success("Notas salvas");
        }}><Save className="mr-1 h-3.5 w-3.5" />Salvar notas</Button>
      </div>
    </div>
  );
}
...
function ToothSummary({ teeth, onPick }: { teeth: ToothRow[]; onPick: (n: number) => void }) {
  const rows = teeth.filter((t) => t.status !== "sadio" || t.procedure || t.notes || (t.images?.length ?? 0) > 0)
  ...
  return (
    <div className="rounded-xl border border-border bg-card/60 p-4">
      ...
      <ul className="grid gap-1.5 md:grid-cols-2">
        {rows.map((t) => (
          <li key={t.id} onClick={() => onPick(t.tooth_number)}
            ...
            <Badge variant="outline" className="font-mono">{t.tooth_number}</Badge>
            ...
            {(t.images?.length ?? 0) > 0 && <Badge variant="secondary" className="ml-auto text-[10px]">{t.images?.length} foto(s)</Badge>}
          </li>
        ))}
      </ul>
    </div>
...
function ToothDialog({
  toothNumber, existing, onClose, onSave,
}: {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Dente {toothNumber} <span className="ml-2 text-xs font-normal text-muted-foreground">Triadan</span></DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button disabled={saving} className="bg-gradient-primary" onClick={async () => {
            setSaving(true);
