...
import {
  addDays, addWeeks, format, isSameDay, startOfWeek, subWeeks,
} from "date-fns";
...
import { useRxQuery, useRxCollection, uuid, getCollection } from "@/hooks/use-rx";
...
const CATEGORIES: { value: string; label: string; color: string }[] = [
  ...
  { value: "cirurgia", label: "Cirurgia", color: "bg-destructive/20 text-destructive border-destructive/40" },
  { value: "vacina", label: "Vacina", color: "bg-success/20 text-success border-success/40" },
  { value: "odonto", label: "Odonto", color: "bg-accent/20 text-accent-foreground border-accent/40" },
...
function AgendaPage() {
  ...
  const [form, setForm] = useState({
    title: "", category: "consulta", date: format(new Date(), "yyyy-MM-dd"),
    time: "09:00", duration_min: "30", client_id: "", pet_id: "", notes: "",
  });
  ...
  async function save() {
    if (!form.title.trim()) { toast.error("Informe o título do agendamento."); return; }
    if (!apptsCol) return;
    ...
    try {
      ...
      if (organizationId) {
        syncAppointmentToGoogle(id, organizationId);
      }
  ...
  async function remove(id: string) {
    ...
    try {
      ...
      if (doc) {
        const googleEventId = doc.google_event_id;
        await doc.remove();
        if (googleEventId && organizationId) {
          deleteGoogleEvent(googleEventId, organizationId);
        }
      }
    } catch (e) { toast.error((e as Error).message); }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {isSameDay(anchor, new Date()) 
              ...
              : `Exibindo semana de ${format(anchor, "dd/MM/yyyy", { locale: ptBR })}`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button 
            variant={isSameDay(anchor, new Date()) ? "secondary" : "outline"} 
            size="sm" 
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="gap-1.5"><Plus className="h-4 w-4" /> Novo</Button>
            </DialogTrigger>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-7">
        {days.map((d, i) => {
          const dayAppts = appts.filter((a) => isSameDay(new Date(a.scheduled_at), d))
          ...
          return (
            <motion.div
              ...
              className={`glass flex min-h-48 flex-col rounded-xl p-3 shadow-elegant cursor-pointer transition-all ${isToday ? "ring-1 ring-primary/40" : ""} ${isSameDay(d, anchor) ? "bg-primary/5 border-primary/30" : ""}`}
              onClick={() => setAnchor(d)}
            >
              <div className="mb-2 flex items-center justify-between">
                <div>
                  ...
                  <div className={`font-display text-lg font-semibold ${isToday ? "text-primary" : ""}`}>
                    {format(d, "d")}
                  </div>
                </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        {CATEGORIES.map((c) => (
          <span key={c.value} className={`rounded-md border px-2 py-1 ${c.color}`}>{c.label}</span>
        ))}
      </div>
    </div>
  );
