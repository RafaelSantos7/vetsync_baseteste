...
const empty = {
  client_id: "", name: "", species: "cao", breed: "", sex: "M", birth_date: "",
  weight: "", color: "", neutered: false, allergies: "", diseases: "", medications: "",
...
function AnimaisPage() {
  ...
  const { data: pets = [], isLoading: rxLoading } = useRxQuery<Pet>("pets", {
    selector: { 
      is_deleted: { $ne: true },
  ...
  // Não exibir "nenhum registro" antes da primeira sincronização com o servidor.
  const isLoading = authLoading || rxLoading || !initialSyncDone;
  ...
  function openEdit(p: Pet) {
    setEditing(p);
    setForm({
      client_id: p.client_id, name: p.name, species: p.species ?? "cao",
      breed: p.breed ?? "", sex: p.sex ?? "M", birth_date: p.birth_date ?? "",
      weight: p.weight?.toString() ?? "", color: p.color ?? "",
      neutered: !!p.neutered, allergies: p.allergies ?? "",
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-xl w-[95dvw] h-[90dvh] sm:h-auto overflow-y-auto">
            ...
            <div className="grid gap-4 sm:grid-cols-2 pb-20 sm:pb-0">
              ...
              <F label="Peso (kg)" v={form.weight} on={(v) => setForm({ ...form, weight: v })} type="number" />
              ...
              <div className="sm:col-span-2">
                ...
                <Textarea value={form.diseases} onChange={(e) => setForm({ ...form, diseases: e.target.value })} rows={2} />
              </div>
              <div className="sm:col-span-2">
                <Label className="text-xs">Medicações em uso</Label>
                <Textarea value={form.medications} onChange={(e) => setForm({ ...form, medications: e.target.value })} rows={2} />
              </div>
              <div className="sm:col-span-2">
          ...
          {filtered.map((p, i) => (
            <motion.div key={p.id}
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{clientMap.get(p.client_id) ?? "—"}</p>
                  </div>
                </div>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {p.species && <Badge variant="secondary" className="text-[10px]">{p.species}</Badge>}
                {p.breed && <Badge variant="outline" className="text-[10px]">{p.breed}</Badge>}
                {p.sex && <Badge variant="outline" className="text-[10px]">{p.sex === "M" ? "Macho" : "Fêmea"}</Badge>}
                {p.weight && <Badge variant="outline" className="text-[10px]">{p.weight} kg</Badge>}
              </div>
            </motion.div>
          ))}
        </div>
